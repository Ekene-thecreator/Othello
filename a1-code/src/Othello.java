...
public class Othello {
	...
	public boolean move(int row, int col) {
		...
		if (moves == OthelloBoard.BOTH || moves == this.getWhoseTurn()) {
			board.move(row, col, this.getWhoseTurn());
			this.whoseTurn = OthelloBoard.otherPlayer(this.getWhoseTurn());
			this.numMoves += 1;
			return true;
		}
	...
	public char getWinner() {
		...
		if (this.isGameOver() == true) {
			int p_1 = board.getCount(OthelloBoard.P1);
			int p_2 = board.getCount(OthelloBoard.P2);
			if (p_1 > p_2) {
