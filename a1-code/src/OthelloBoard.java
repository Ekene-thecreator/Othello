...
public class OthelloBoard {
	...
	private char checkForPattern(int row, int col, int drow, int dcol) {
		...
		if ((drow == 0 && dcol == 0)||!(row>=0 && row<8 && col>=0 && col<8)||(this.board[row][col])== EMPTY) {
			return EMPTY;
		}
		int crow = row ;
		int ccol = col;
		while ((crow+drow>=0 && crow+drow<8 && ccol+dcol>=0 && ccol+dcol<8) && this.board[crow][ccol] == this.board[crow + drow][ccol + dcol]) {
			crow += drow;
			ccol += dcol;
		}
		if (crow+drow>=0 && crow+drow<8 && ccol+dcol>=0 && ccol+dcol<8) {
			return this.board[crow + drow ][ccol + dcol];
		}
		return EMPTY;
	}
	...
	private int flip(int row, int col, int drow, int dcol, char player) {
		...
		if (this.checkForPattern(row, col, drow, dcol) == player){
			while ((crow+drow>=0 && crow+drow<8 && ccol+dcol>=0 && ccol+dcol<8) && this.board[crow][ccol] != player) {
	...
	public boolean move(int row, int col, char player) {
		...
		for (int drow = -1; drow <=1; drow ++) {
			for (int dcol = -1; dcol <= 1; dcol ++) {
				if (this.checkForPattern(row + drow, col + dcol, drow, dcol) == player) {
